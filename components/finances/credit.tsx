...
export function CreditHealth() {
  ...
  const [open, setOpen] = useState<string | null>(null); // which add-form is open
  ...
  const done = useCallback(() => { setOpen(null); void load(); }, [load]);
  ...
  return (
    <div className="fin-credit">
      ...
      {tab === "profile" && <ProfileTab view={view} open={open} setOpen={setOpen} done={done} />}
      {tab === "goals" && <GoalsTab view={view} open={open} setOpen={setOpen} done={done} />}
...
/* -------------------------------------------------------------- Profile -- */
function ProfileTab({ view, open, setOpen, done }: { view: Overview; open: string | null; setOpen: (s: string | null) => void; done: () => void }) {
  return (
    <div className="fin-credit-body">
      {/* Scores */}
      <Panel title="Scores" add="score" open={open} setOpen={setOpen}
        form={<ScoreForm accounts={view.accounts} done={done} />}>
        {view.scores.map((s) => <li key={s.id} className="fin-credit-row"><span>{s.score} · {s.source}{s.scoringModel ? ` (${s.scoringModel})` : ""}</span><span className="num">as of {s.asOfDate}</span></li>)}
      </Panel>
      {/* Accounts */}
      <Panel title="Accounts, limits &amp; balances" add="account" open={open} setOpen={setOpen}
        form={<AccountForm done={done} />}>
        {view.accounts.map((a) => <li key={a.id} className="fin-credit-row"><span>{a.name} · {a.accountType.replace(/_/g, " ")}{a.isRevolving ? " · revolving" : ""}{a.isAuthorizedUser ? " · authorized user" : ""} · {a.status}</span><span className="num">{money(Number(a.currentBalance))}{a.creditLimit ? ` / ${money(Number(a.creditLimit))}` : " (no limit)"}</span></li>)}
      </Panel>
      {/* Utilization summary */}
      ...
      {/* Collections */}
      <Panel title="Collections" add="collection" open={open} setOpen={setOpen}
        form={<CollectionForm done={done} />}>
        {view.collections.map((c) => <li key={c.id} className="fin-credit-row"><span>{c.collectorName}{c.originalCreditor ? ` (orig. ${c.originalCreditor})` : ""} · {c.status} · {c.validationStatus.replace(/_/g, " ")}</span><span className="num">{money(Number(c.reportedBalance))}</span></li>)}
      </Panel>
      ...
      {/* Inquiries */}
      <Panel title="Inquiries" add="inquiry" open={open} setOpen={setOpen}
        form={<InquiryForm done={done} />}>
        {view.inquiries.map((i) => <li key={i.id} className="fin-credit-row"><span>{i.creditorName} · {i.inquiryType}{i.bureau ? ` · ${i.bureau}` : ""}</span><span className="num">{i.inquiryDate}</span></li>)}
      </Panel>
      ...
      {/* Late payments */}
      <Panel title="Late-payment records" add="late" open={open} setOpen={setOpen}
        form={<LateForm accounts={view.accounts} done={done} />}>
        {view.latePayments.map((l) => <li key={l.id} className="fin-credit-row"><span>{view.accounts.find((a) => a.id === l.creditAccountId)?.name ?? `account #${l.creditAccountId}`} · {l.daysLate} days late · {l.status}</span><span className="num">{l.reportedDate}</span></li>)}
      </Panel>
    </div>
  );
}
...
/* ---------------------------------------------------------------- Goals -- */
function GoalsTab({ view, open, setOpen, done }: { view: Overview; open: string | null; setOpen: (s: string | null) => void; done: () => void }) {
  return (
    <div className="fin-credit-body">
      <Panel title="Credit goals" add="goal" open={open} setOpen={setOpen} form={<GoalForm done={done} />}>
        {view.goalProgress.map((g) => <li key={g.id} className="fin-credit-row"><span>{g.goalType.replace(/_/g, " ")} → {g.targetValue} · {g.status}</span><span className="num">{g.progressPct != null ? `${Math.round(g.progressPct)}%` : "—"}{g.onTrack != null ? (g.onTrack ? " · on track" : " · behind") : ""}</span></li>)}
      </Panel>
...
/* -------------------------------------------------------------- forms ---- */
function Panel({ title, add, open, setOpen, form, children }: { title: string; add: string; open: string | null; setOpen: (s: string | null) => void; form: React.ReactNode; children: React.ReactNode }) {
  return (
    <div className="fin-credit-panel">
      <div className="fin-credit-panel-head">
        ...
        <button className="fin-mini-btn" onClick={() => setOpen(open === add ? null : add)}>{open === add ? "Cancel" : "+ Add"}</button>
      </div>
      {open === add && <div className="fin-credit-form">{form}</div>}
      <ul className="fin-credit-list">{children}</ul>
    </div>
  );
}
...
function ScoreForm({ done }: { accounts: Account[]; done: () => void }) {
  const [f, setF] = useState({ score: "", source: "experian", scoringModel: "", asOfDate: "", notes: "" });
  const [err, setErr] = useState<string | null>(null);
  return (
    <form onSubmit={async (e) => { e.preventDefault(); const ok = await post("/api/finances/credit/scores", { ...f, score: Number(f.score) }); if (ok) done(); else setErr("Could not save — check the score range and date."); }}>
      ...
      {err && <p className="fin-credit-warn">{err}</p>}
      <button className="fin-mini-btn" type="submit">Save score</button>
    </form>
  );
}
function AccountForm({ done }: { done: () => void }) {
  const [f, setF] = useState({ name: "", accountType: "credit_card", isRevolving: true, creditLimit: "", currentBalance: "", minimumPayment: "", paymentDueDate: "", isAuthorizedUser: false, status: "open" });
  const [err, setErr] = useState<string | null>(null);
  return (
    <form onSubmit={async (e) => { e.preventDefault(); const ok = await post("/api/finances/credit/accounts", { ...f, creditLimit: f.creditLimit || null, currentBalance: f.currentBalance || 0, minimumPayment: f.minimumPayment || null, paymentDueDate: f.paymentDueDate || null }); if (ok) done(); else setErr("Could not save — revolving accounts need a positive limit; balances cannot be negative."); }}>
...
function CollectionForm({ done }: { done: () => void }) {
  const [f, setF] = useState({ collectorName: "", originalCreditor: "", reportedBalance: "", status: "reported", validationStatus: "not_requested", dateReported: "" });
  const [err, setErr] = useState<string | null>(null);
  return (
    <form onSubmit={async (e) => { e.preventDefault(); const ok = await post("/api/finances/credit/collections", { ...f, reportedBalance: f.reportedBalance || 0, originalCreditor: f.originalCreditor || null, dateReported: f.dateReported || null }); if (ok) done(); else setErr("Could not save the collection."); }}>
      ...
      <Row label="Validation status"><select value={f.validationStatus} onChange={(e) => setF({ ...f, validationStatus: e.target.value })}>{["not_requested", "requested", "received", "incomplete", "verified_by_owner"].map((s) => <option key={s} value={s}>{s.replace(/_/g, " ")}</option>)}</select></Row>
      {err && <p className="fin-credit-warn">{err}</p>}
...
function InquiryForm({ done }: { done: () => void }) {
  const [f, setF] = useState({ creditorName: "", inquiryDate: "", inquiryType: "hard", bureau: "" });
  const [err, setErr] = useState<string | null>(null);
  return (
    <form onSubmit={async (e) => { e.preventDefault(); const ok = await post("/api/finances/credit/inquiries", { ...f, bureau: f.bureau || null }); if (ok) done(); else setErr("Could not save — this inquiry may already exist."); }}>
...
function LateForm({ accounts, done }: { accounts: Account[]; done: () => void }) {
  const [f, setF] = useState({ creditAccountId: "", daysLate: "30", reportedDate: "", status: "reported" });
  const [err, setErr] = useState<string | null>(null);
  return (
    <form onSubmit={async (e) => { e.preventDefault(); const ok = await post("/api/finances/credit/late-payments", { ...f, creditAccountId: Number(f.creditAccountId), daysLate: Number(f.daysLate) }); if (ok) done(); else setErr("Could not save — select an account and valid days late."); }}>
      <Row label="Account"><select required value={f.creditAccountId} onChange={(e) => setF({ ...f, creditAccountId: e.target.value })}><option value="">Select…</option>{accounts.map((a) => <option key={a.id} value={a.id}>{a.name}</option>)}</select></Row>
...
function GoalForm({ done }: { done: () => void }) {
  const [f, setF] = useState({ goalType: "score_target", targetValue: "", targetDate: "", priority: "medium" });
  const [err, setErr] = useState<string | null>(null);
  return (
    <form onSubmit={async (e) => { e.preventDefault(); const ok = await post("/api/finances/credit/goals", { ...f, targetValue: Number(f.targetValue), targetDate: f.targetDate || null }); if (ok) done(); else setErr("Could not save — check the target value for this goal type."); }}>
